...
namespace SportSync.Application.Events.RejectEventInvitation;

public class RejectEventInvitationRequestHandler : IRequestHandler<EventInvitationInput, Result>
{
    ...
    public RejectEventInvitationRequestHandler(
    ...
    public async Task<Result> Handle(EventInvitationInput request, CancellationToken cancellationToken)
    {
        ...
        Result acceptResult = @event.RejectInvitation(request.EventInvitationId, user, _dateTime.UtcNow);
